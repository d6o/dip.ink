...
Public runtime images are **immutable tags**, not moving `latest`. v0.1.1 is the first public release pin.
...
├── docker-compose.yml      ← the whole stack on one host (pinned v0.1.1 images)
├── .env.example
...
`deploy/k8s/` mirrors the compose stack for a cluster: Neo4j Deployment + PVC, the memory-server Deployment, the 15-min ingest CronJob, and the maintenance CronJobs. Images are published to GHCR by this repo's CI and **pinned to `v0.1.1`**.
...
- Public manifests and template workflows pin `ghcr.io/d6o/dip.ink/{memory,pi-runner}:v0.1.1`.

Release sequence for maintainers: push main → wait CI green → tag `v0.1.1` → wait image publication green → sync private instances / deploy.
