...
1. Start with one batched shell pass that inventories live folders, identifies note/attachment files, reads their contents, and checks dedup logs. Do not spend separate turns rediscovering the same inputs.
...
- **FLAG** only for corrupt or unparseable input. Leave the folder live and log the reason.
